...
fn main() {
    ...
    for round in 1..=num_rounds {
        ...
        let initial_path = if rng.random_bool(0.5) { "A" } else { "B" };

        // The Verifier asks for a random path to return
        let requested_path = if rng.random_bool(0.5) { "A" } else { "B" };
        ...
        println!(
            "Round {}: The Prover starts at {}, The Verifier requests {} - Peggy {}",
            round,
            initial_path,
            requested_path,
